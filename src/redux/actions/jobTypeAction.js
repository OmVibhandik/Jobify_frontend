...
export const jobTypeLoadAction = () => async (dispatch) => {
    ...
    try {
        const { data } = await axios.get(process.env.REACT_APP_API_URL+'/api/type/jobs');
        dispatch({
...
export const createJobTypeAction = (jobtype) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.post(process.env.REACT_APP_API_URL+"/api/type/create", jobtype)
        dispatch({
